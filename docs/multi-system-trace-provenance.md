...
Portfolio Query is a second independently compiled graph that consumes an immutable indexed snapshot by ID. Its manifest shares the Portfolio Analyst product identity while retaining a separate flow key and compatibility runtime key. The full snapshot is resolved through request-scoped runtime context and only a typed allowlist is sent to inference. [The flow refactor plan](agent-flow-refactor-plan.md) normalizes both flows under one persistent Portfolio Analyst product without merging their nodes.
